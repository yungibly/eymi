...
The rendering suite checks rendered Markdown in real cells at 120×40 and 80×24: table grids with a right-aligned column, a labelled and highlighted code surface, heading margin marks, alert titles, struck checked tasks, quote rails, and rules. It also checks that clicking a cell reveals the table's source, that source view numbers its lines, and that every view saves the original bytes.
